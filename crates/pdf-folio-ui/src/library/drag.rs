...
#[cfg(test)]
use iced::Rectangle;
use pdf_folio_library::{EntryId, Folder, FolderId, LibrarySortMode};

pub(crate) const LIBRARY_DRAG_AUTOSCROLL_TICK_MS: u64 = 16;
pub(crate) const LIBRARY_DRAG_AUTOSCROLL_EDGE_BAND: f32 = 96.0;
pub(crate) const LIBRARY_DRAG_AUTOSCROLL_MAX_SPEED: f32 = 980.0;
pub(crate) const LIBRARY_DRAG_AUTOSCROLL_MIN_SPEED: f32 = 80.0;
pub(crate) const LIBRARY_DRAG_AUTOSCROLL_MAX_DT: f32 = 1.0 / 20.0;
pub(crate) const LIBRARY_DRAG_ACTIVATION_DISTANCE: f32 = 6.0;
pub(crate) const LIBRARY_FOLDER_DROP_DWELL_MS: u64 = 500;
pub(crate) const LIBRARY_FOLDER_DROP_FLASH_MS: u64 = 900;
...
impl LibraryDragState {
    ...
    pub(crate) fn update_cursor(&mut self, cursor: Point) -> bool {
    ...
    pub(crate) fn set_pending_folder_target(
        &mut self,
    ...
    pub(crate) fn pending_target_ready(&self, now: Instant) -> Option<FolderId> {
...
impl FolderDragState {
    pub(crate) fn new(folder_id: FolderId) -> Self {
        Self {
            folder_id,
            drop_target: None,
            pending_drop_target: None,
...
pub(crate) fn can_drag_reorder_library(
...
pub(crate) fn active_folder_drop_target<'a>(
...
pub(crate) fn drag_auto_scroll_velocity(
    cursor_y: f32,
    viewport_y: f32,
    viewport_height: f32,
) -> f32 {
...
pub(crate) fn distance_between(a: Point, b: Point) -> f32 {
    let dx = a.x - b.x;
    let dy = a.y - b.y;
    (dx * dx + dy * dy).sqrt()
}

pub(crate) fn reorder_folder_ids_before_target(
    folders: &[FolderId],
...
pub(crate) fn folder_can_move_into(
    folders: &[Folder],
    folder_id: &FolderId,
    target_id: &FolderId,
) -> bool {
...
#[cfg(test)]
pub(crate) fn folder_drop_target_at_cursor(
    cursor: Point,
