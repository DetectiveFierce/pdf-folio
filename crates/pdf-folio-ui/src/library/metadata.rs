...
use pdf_folio_library::LibraryEntry;

use super::state::LibraryMetadataDensity;

pub(crate) fn entry_title(entry: &LibraryEntry) -> String {
    entry
        .display_title
        .clone()
...
pub(crate) fn entry_author(entry: &LibraryEntry) -> String {
    entry
        .display_author
        .clone()
...
pub(crate) fn clean_metadata_input(value: &str) -> Option<String> {
...
pub(crate) fn page_count_label(entry: &LibraryEntry) -> String {
...
pub(crate) fn last_opened_label(entry: &LibraryEntry) -> String {
...
pub(crate) fn file_size_label(entry: &LibraryEntry) -> String {
    entry
        .file_size
        .map_or_else(|| String::from("Unknown size"), format_file_size)
}

pub(crate) fn total_file_size_label(entries: &[LibraryEntry]) -> String {
    let mut total = 0_u64;
...
pub(crate) fn library_card_metadata_label(
...
pub(crate) fn library_row_metadata_label(
    density: LibraryMetadataDensity,
    entry: &LibraryEntry,
) -> String {
    match density {
...
pub(crate) fn library_card_page_count_label(entry: &LibraryEntry) -> String {
    entry.page_count.map_or_else(
        || String::from("Unknown pages"),
        |pages| {
            if pages == 1 {
...
pub(crate) fn progress_fraction(entry: &LibraryEntry) -> f32 {
    if entry.missing {
        return 0.0;
    }
