...
impl Direction {
    ...
    fn align(&self, delta: Vector) -> Vector {
        ...
        Vector::new(align(horizontal_alignment, delta.x), delta.y)
    }
}
    ...
    fn translation(
        ...
        Vector::new(
            ...
            if direction.vertical().is_some() {
                self.offset_y
                    .translation(bounds.height, content_bounds.height, Anchor::Start)
                    .round()
            } else {
                0.0
            },
...
impl Scrollbars {
    ...
    fn new(
        ...
        let y_scrollbar = if let Some(vertical) = show_scrollbar_y {
            ...
            let left_aligned = vertical.alignment == Anchor::End;
            let total_scrollbar_x = if left_aligned {
                bounds.x
            } else {
                bounds.x + bounds.width - total_scrollbar_width
            };
            let scrollbar_x = if left_aligned {
                bounds.x + total_scrollbar_width / 2.0 - width / 2.0
            } else {
                bounds.x + bounds.width - total_scrollbar_width / 2.0 - width / 2.0
            };
            let scroller_x = if left_aligned {
                bounds.x + total_scrollbar_width / 2.0 - scroller_width / 2.0
            } else {
                bounds.x + bounds.width - total_scrollbar_width / 2.0 - scroller_width / 2.0
            };
