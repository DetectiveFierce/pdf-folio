...
use crate::style::MasterCheckboxState;

pub(crate) fn range_selection_ids(
    first_index: usize,
    second_index: usize,
    entry_ids: &[EntryId],
) -> Vec<EntryId> {
...
pub(crate) fn toggle_selection_entry_id(selection: &mut HashSet<EntryId>, entry_id: EntryId) {
    if !selection.insert(entry_id.clone()) {
        selection.remove(&entry_id);
    }
}

pub(crate) fn master_checkbox_state_for_counts(
...
pub(crate) fn reorder_entry_ids_for_drag(
    entries: &[EntryId],
...
#[cfg(test)]
pub(crate) fn dragged_placeholder_count(entries: &[EntryId], dragged_entries: &[EntryId]) -> usize {
